...
(function oneko() {
  ...
  function init() {
    ...
    // Tint the white cat to red while keeping black outlines
    nekoEl.style.filter = "sepia(1) saturate(1000%) hue-rotate(-50deg)";
  ...
  function idle() {
    ...
    // Make it lazier (more likely to idle) if moving slowly
    let idleChance = nekoSpeed < 10 ? 50 : 200;
    if (
      idleTime > 10 &&
      Math.floor(Math.random() * idleChance) == 0 &&
      idleAnimation == null
    ) {
  ...
  function frame() {
    ...
    // Random mood swings for speed and laziness
    if (Math.random() < 0.005) {
      const mood = Math.random();
      if (mood < 0.15) nekoSpeed = 25; // Zoomies!
      else if (mood < 0.35) nekoSpeed = 4; // Lazy / slow
      else nekoSpeed = 10; // Normal
    }
    ...
    let stopDistance = nekoSpeed < 10 ? 96 : 48;
