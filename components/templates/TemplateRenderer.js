...
function BirthdaySurpriseTemplate({ note, isPreview = false }) {
  const [phase, setPhase] = useState(1); // 1=balloons, 2=cake+candles, 3=letter
  ...
  const ageMilestone = note?.custom_details?.age_milestone || '';

  const balloonColors = ['#ff4b8b', '#ffd700', '#38bdf8', '#a855f7', '#34d399', '#fb923c'];
  const balloons = [0, 1, 2, 3, 4, 5];
  const allBalloonPopped = poppedBalloons.length === balloons.length;
  const allCandlesBlown = candlesBlown.every(Boolean);

  // Trigger confetti and advance phase when all balloons popped
  useEffect(() => {
    if (allBalloonPopped && phase === 1) {
      setShowConfetti(true);
      const t = setTimeout(() => { setPhase(2); setShowConfetti(false); }, 1800);
      return () => clearTimeout(t);
    }
  }, [allBalloonPopped, phase]);

  // Trigger confetti and advance when all candles blown
  useEffect(() => {
    if (allCandlesBlown && phase === 2) {
      setShowConfetti(true);
      const t = setTimeout(() => { setPhase(3); }, 1600);
      return () => clearTimeout(t);
    }
  }, [allCandlesBlown, phase]);
  ...
  const handleBlowCandle = (idx) => {
    setCandlesBlown(prev => { const n = [...prev]; n[idx] = true; return n; });
  };

  return (
    <div style={{ background: 'linear-gradient(135deg, #1a0533, #3b0764)', borderRadius: '24px', padding: '2rem 1.5rem', maxWidth: '600px', margin: '0 auto', textAlign: 'center', minHeight: '500px', color: '#fff', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'center', marginBottom: '1.5rem' }}>
        {['🎈', '🎂', '💌'].map((icon, i) => (
          <span key={i} style={{ width: '32px', height: '32px', borderRadius: '50%', background: phase === i + 1 ? '#ffd700' : phase > i + 1 ? '#f59e0b' : 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', transition: 'all 0.3s ease' }}>{icon}</span>
        ))}
      </div>

      <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#ffd700', letterSpacing: '0.12em', textTransform: 'uppercase' }}>🎂 VIRTUAL BIRTHDAY BASH</span>
      <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 2.25rem)', margin: '0.4rem 0' }}>
        Happy Birthday, {recipient}! 🎉
      </h1>
      {ageMilestone && (
        <span style={{ background: '#ffd700', color: '#3b0764', padding: '0.2rem 0.85rem', borderRadius: '999px', fontSize: '0.85rem', fontWeight: 800, display: 'inline-block', marginBottom: '1rem' }}>
          ✨ {ageMilestone} ✨
        </span>
      )}

      {/* PHASE 1: Poppable Balloons */}
      {phase === 1 && (
        <div style={{ marginTop: '1.5rem' }}>
          <p style={{ color: '#fde68a', fontSize: '0.9rem', marginBottom: '1.25rem' }}>
            🎈 Pop all {balloons.length} balloons to reveal the cake!
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'center', marginBottom: '1.5rem' }}>
            {balloons.map(idx => (
              <button
                ...
                style={{
                  fontSize: poppedBalloons.includes(idx) ? '2.5rem' : '3.5rem',
                  background: 'none', border: 'none', cursor: 'pointer',
                  opacity: poppedBalloons.includes(idx) ? 0.25 : 1,
                  transition: 'all 0.3s ease',
                  filter: poppedBalloons.includes(idx) ? 'grayscale(1)' : 'none',
                  transform: poppedBalloons.includes(idx) ? 'scale(0.6)' : 'scale(1)',
                  animation: !poppedBalloons.includes(idx) ? `floatUp ${2 + idx * 0.3}s ease-in-out infinite alternate` : 'none'
                }}
          ...
          <p style={{ color: '#fde68a', fontSize: '0.8rem' }}>
            {poppedBalloons.length}/{balloons.length} balloons popped!
          </p>
          {allBalloonPopped && <p style={{ color: '#34d399', fontWeight: 800, marginTop: '0.5rem' }}>All popped! 🎉 Loading the cake...</p>}
        </div>
      )}

      {/* PHASE 2: Cake + Candles */}
      {phase === 2 && (
        <div style={{ marginTop: '1.5rem' }}>
          <p style={{ color: '#fde68a', fontSize: '0.9rem', marginBottom: '1.25rem' }}>
            🎂 Tap each candle to blow it out and make a wish!
          </p>

          {/* Cake visual with name */}
          <div style={{ position: 'relative', display: 'inline-block', marginBottom: '1rem' }}>
            <div style={{ fontSize: '7rem', lineHeight: 1 }}>🎂</div>
            <div style={{ position: 'absolute', top: '-10px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '0.5rem' }}>
              {candlesBlown.map((blown, idx) => (
                <button
                  key={idx}
                  onClick={() => handleBlowCandle(idx)}
                  style={{ background: 'none', border: 'none', cursor: blown ? 'default' : 'pointer', fontSize: '1.6rem', transition: 'all 0.3s ease' }}
            ...
            <div style={{ position: 'absolute', bottom: '8px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(255,215,0,0.9)', padding: '0.1rem 0.7rem', borderRadius: '999px', fontSize: '0.8rem', fontWeight: 800, color: '#3b0764', whiteSpace: 'nowrap' }}>
              {recipient}
            </div>
          </div>
          ...
          {allCandlesBlown && <p style={{ color: '#34d399', fontWeight: 800, marginTop: '0.5rem' }}>Wish made! 🌟 Opening your card...</p>}
        </div>
      )}

      {/* PHASE 3: Birthday Letter Reveal */}
      {phase === 3 && (
        <div style={{ marginTop: '1.5rem', animation: 'sorryStepFadeIn 0.5s ease' }}>
          <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '20px', padding: '1.5rem', border: '2px solid rgba(255,215,0,0.3)', textAlign: 'left', marginBottom: '1.5rem' }}>
            <h3 style={{ color: '#ffd700', fontSize: '1.25rem', margin: '0 0 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>🎈</span> Dearest {recipient}, <span>⭐</span>
            </h3>
            <p style={{ fontSize: '1.05rem', lineHeight: 1.9, whiteSpace: 'pre-wrap', color: '#fde8ff' }}>
              {typedLetter}
              <span style={{ borderRight: '2px solid #ffd700', marginLeft: '2px', animation: 'blink 1s step-end infinite' }} />
            </p>
          </div>
          ...
          {note?.image_urls?.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
              {note.image_urls.map((url, i) => (
                <div key={i} style={{ borderRadius: '12px', overflow: 'hidden', border: '3px solid #ffd700', boxShadow: '0 4px 12px rgba(255,215,0,0.2)' }}>
                  <img src={url} alt="Birthday Memory" style={{ width: '100%', height: '120px', objectFit: 'cover', display: 'block' }} />
                </div>
              ))}
            </div>
          )}

          <div style={{ fontFamily: 'cursive', fontSize: '1.8rem', color: '#ffd700', textAlign: 'right' }}>
            Sending hugs & sweetness ❤️
          </div>
          <ReplayAndMarketingFooter onReplay={() => { setPhase(1); setPoppedBalloons([]); setCandlesBlown([false, false, false]); setShowConfetti(false); setTypedLetter(''); }} isPreview={isPreview} />
        </div>
      )}
    </div>
  );
