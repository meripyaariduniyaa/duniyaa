...
function BirthdaySurpriseTemplate({ note, isPreview = false }) {
  const [scene, setScene] = useState(1); // 1=Envelope, 2=Welcome, 3=Balloons, 4=MemoryGallery, 5=Candle Blow, 6=Rose, 7=Spin Wheel, 8=Letter, 9=Final
  
  // States for various scenes
  const [showConfetti, setShowConfetti] = useState(false);
  const [runawayYesHovered, setRunawayYesHovered] = useState(false);
  const [runawayNoPos, setRunawayNoPos] = useState({ top: 0, left: 0 });
  ...
  const handleNoHover = () => {
    const randomTop = Math.random() * 150 - 75;
    const randomLeft = Math.random() * 150 - 75;
    setRunawayNoPos({ top: randomTop, left: randomLeft });
  };

  // Balloon popping logic
  const balloons = [0, 1, 2, 3];
    ...
    if (!poppedBalloons.includes(idx)) {
      setPoppedBalloons(prev => {
        ...
        if (next.length === balloons.length) {
          setShowConfetti(true);
          setTimeout(() => { setScene(4); setShowConfetti(false); }, 2500);
        }
        return next;
      });
    }
  ...
  const startMicListener = async () => {
    try {
      ...
      const detectBlow = () => {
        ...
        if (average > 70 && !candleBlown) {
          setCandleBlown(true);
          setTimeout(() => setScene(6), 3000); // give time for smoke animation
          stopMic();
        } else if (!candleBlown) {
  ...
  // Spin wheel logic
  const wishes = ["Free Coffee Treat! ☕", "Unlimited Warm Hugs 🤗", "Movie Night Picker 🍿", "A Special Gift Coming 🎁", "Breakfast in Bed 🥞"];
  const handleSpin = () => {
    setWheelSpun(true);
    setTimeout(() => {
      setWheelResult(wishes[Math.floor(Math.random() * wishes.length)]);
      setShowConfetti(true);
  ...
  useEffect(() => {
    if (scene === 8) {
      ...
      let i = 0;
      const t = setInterval(() => {
        if (i < customMsg.length) {
          setTypedLetter(prev => prev + customMsg.charAt(i));
          i++;
  ...
  const handleReplay = () => {
    ...
    setRunawayNoPos({ top: 0, left: 0 });
  };

  return (
    <div className="app-container birthday-new-theme">
      {/* Floating hearts background */}
      <div className="heart-bg" style={{ left: '10%' }}>💖</div>
      <div className="heart-bg" style={{ left: '40%', animationDelay: '2s' }}>✨</div>
      <div className="heart-bg" style={{ left: '70%', animationDelay: '4s' }}>💕</div>

      {scene === 5 && candleBlown && <div className="overlay-dark" style={{ opacity: 1 }}></div>}
      ...
      <button 
        onClick={toggleMusic}
        style={{ position: 'absolute', top: '15px', right: '15px', background: 'rgba(255,255,255,0.3)', border: '1px solid rgba(255,255,255,0.6)', borderRadius: '999px', padding: '6px 12px', color: '#db2777', fontSize: '0.8rem', cursor: 'pointer', zIndex: 50, backdropFilter: 'blur(5px)', fontWeight: 'bold' }}
      >
        {isPlayingAudio ? '🎵 Music: ON' : '🎵 Music: OFF'}
      </button>
      <audio ref={bgAudioRef} loop src="https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3?filename=romantic-acoustic-guitar-113264.mp3" />

      {/* SCENE 1: QR Envelope / Entry Gate */}
      {scene === 1 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem' }}>
          <div className={`qr-card pulse-element`} onClick={() => setScene(2)} style={{ cursor: 'pointer' }}>
            <div style={{ fontSize: '5rem', marginBottom: '1rem' }}>✉️</div>
            <h2 style={{ fontSize: '1.5rem', color: '#db2777', fontFamily: 'var(--font-handwriting)' }}>For {recipient}</h2>
            <p style={{ color: '#831843', fontSize: '0.9rem', marginTop: '0.5rem' }}>Tap to open</p>
          </div>
        </div>
      )}

      {/* SCENE 2: Welcome + Runaway "No" Button */}
      {scene === 2 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', textAlign: 'center', animation: 'fadeIn 0.5s ease' }}>
          <h1 style={{ fontSize: '2.5rem', color: '#db2777', fontFamily: 'var(--font-handwriting)' }}>Happy Birthday, {recipient}! 🎉</h1>
          <div className="pulse-element" style={{ fontSize: '4rem', margin: '2rem 0' }}>🥳</div>
          <h2 style={{ fontSize: '1.3rem', color: '#831843', marginBottom: '1rem' }}>Are you excited for what's next?</h2>
          
          <div className="button-group">
            <button 
              className="btn-yes"
              onClick={() => setScene(3)}
              style={{ transform: runawayYesHovered ? 'scale(1.05)' : 'scale(1)' }}
              onMouseEnter={() => setRunawayYesHovered(true)}
              onMouseLeave={() => setRunawayYesHovered(false)}
            >
              YES! Let's Go!
            </button>
            ...
            <button 
              ...
              style={{ top: `${runawayNoPos.top}px`, left: `${runawayNoPos.left}px` }}
            >
              No
            </button>
          </div>
      ...
      {/* SCENE 3: Balloon Pop Reveal */}
      {scene === 3 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <p style={{ color: '#831843', fontSize: '1.1rem', marginBottom: '1rem', fontWeight: 'bold' }}>
            🎈 Pop the balloons! ({poppedBalloons.length}/{balloons.length})
          </p>
          <div className="balloon-grid">
            {balloons.map((idx) => (
              <div key={idx} style={{ position: 'relative', width: '80px', height: '100px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                {!poppedBalloons.includes(idx) ? (
                  <div
                    className="balloon"
          ...
          {poppedBalloons.length === balloons.length && (
            <h2 className="revealed-text" style={{ marginTop: '2rem', fontSize: '1.5rem', color: '#db2777' }}>You are so special! ❤️</h2>
          )}
        </div>
      )}

      {/* SCENE 4: Memory Gallery / Film Roll */}
      {scene === 4 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <h2 style={{ fontSize: '2rem', color: '#db2777', fontFamily: 'var(--font-handwriting)', marginBottom: '0.5rem' }}>Our Memories 📸</h2>
          <p style={{ color: '#831843', fontSize: '0.9rem', marginBottom: '2rem' }}>Tap the photo to flip!</p>
          ...
          <div 
            ...
            style={{ width: '260px', height: '300px', background: '#fff', padding: '12px 12px 45px 12px', borderRadius: '4px', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', cursor: 'pointer', position: 'relative', transform: `rotate(${Math.random() * 6 - 3}deg)`, transition: 'transform 0.3s' }}
          >
            <img src={photos[galleryIndex]} alt="Memory" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '2px' }} />
            <div style={{ position: 'absolute', bottom: '12px', left: 0, width: '100%', textAlign: 'center', color: '#333', fontFamily: 'var(--font-handwriting)', fontSize: '1.4rem' }}>
              {galleryIndex + 1} / {photos.length}
            </div>
          </div>
          <p style={{ color: '#db2777', fontSize: '0.8rem', marginTop: '2rem', fontWeight: 'bold' }}>
            {galleryIndex === photos.length - 1 ? 'Tap one last time to continue...' : 'Tap photo to flip'}
          </p>
        </div>
      )}

      {/* SCENE 5: Interactive Candle Blow (Web Audio API) */}
      {scene === 5 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease', zIndex: 10 }}>
          <h2 style={{ fontSize: '2rem', color: candleBlown ? '#fff' : '#db2777', fontFamily: 'var(--font-handwriting)', marginBottom: '2rem', transition: 'color 1s ease' }}>Make a wish! 🌟</h2>
          
          <div className="candle-wrapper" style={{ marginBottom: '3rem' }}>
            <div style={{ display: 'flex', position: 'absolute', top: '-25px' }}>
               <div className={`flame ${candleBlown ? 'extinguished' : ''}`}></div>
               {candleBlown && <div className="smoke"></div>}
            </div>
          ...
          {!candleBlown ? (
            <>
              {!micError ? (
                <p style={{ color: '#831843', fontSize: '1.2rem', fontWeight: 'bold' }} className="pulse-element">
                  Blow into your microphone! 🌬️
                </p>
              ) : (
                <p style={{ color: '#e11d48', fontSize: '1rem', marginBottom: '1rem' }}>
              ...
              <button 
                onClick={() => {
                  ...
                  setCandleBlown(true);
                  setTimeout(() => setScene(6), 2500);
                }}
                className="btn-yes"
                style={{ marginTop: '2rem', background: 'rgba(219, 39, 119, 0.8)' }}
              >
                Fallback: Tap to blow out
              </button>
            </>
          ) : (
            <h3 style={{ color: '#fff', fontSize: '1.5rem', fontFamily: 'var(--font-handwriting)', animation: 'fadeIn 0.5s ease', marginTop: '1rem' }}>
              Wish sealed! ✨
            </h3>
          )}
        </div>
      ...
      {/* SCENE 6: Interactive Rose Bouquet & Message Bubbles */}
      {scene === 6 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <h2 style={{ fontSize: '2rem', color: '#db2777', fontFamily: 'var(--font-handwriting)', marginBottom: '1rem' }}>A bouquet for you 💐</h2>
          
          <div className="rose-container" onClick={() => setScene(7)}>
            <div className="pulse-element" style={{ fontSize: '6rem' }}>
              💐
            </div>
            
            {/* Floating Message Pills */}
            <div className="message-pill" style={{ top: '10%', left: '0%' }}>Beautiful ✨</div>
            <div className="message-pill" style={{ top: '80%', left: '10%', animationDelay: '1s' }}>Special ❤️</div>
            <div className="message-pill" style={{ top: '20%', right: '0%', animationDelay: '0.5s' }}>Amazing 🌟</div>
            <div className="message-pill" style={{ top: '70%', right: '5%', animationDelay: '1.5s' }}>Loved 💕</div>
          </div>
          
          <p style={{ color: '#831843', fontSize: '1rem', fontWeight: 'bold' }}>Tap the bouquet to continue ➔</p>
        </div>
      )}

      {/* SCENE 7: "Spin the Wheel" / Wish Generator */}
      {scene === 7 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <h2 style={{ fontSize: '2rem', color: '#db2777', fontFamily: 'var(--font-handwriting)', marginBottom: '1rem' }}>Birthday Privilege Wheel 🎡</h2>
          <p style={{ color: '#831843', fontSize: '0.9rem', marginBottom: '2rem' }}>Spin for a special coupon!</p>
          
          <div style={{ position: 'relative', width: '200px', height: '200px', borderRadius: '50%', background: 'conic-gradient(#f43f5e, #ec4899, #d946ef, #8b5cf6, #3b82f6, #06b6d4, #10b981, #f43f5e)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 30px rgba(236, 72, 153, 0.4)', transition: 'transform 1.5s cubic-bezier(0.2, 0.8, 0.2, 1)', transform: wheelSpun ? 'rotate(1080deg)' : 'rotate(0deg)' }}>
            <div style={{ width: '150px', height: '150px', borderRadius: '50%', background: '#fff0f5', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#db2777', fontSize: '1.2rem', fontWeight: 'bold', padding: '20px', textAlign: 'center' }}>
              {!wheelSpun ? '?' : '🎁'}
            </div>
            <div style={{ position: 'absolute', top: '-15px', fontSize: '2rem' }}>🔻</div>
          </div>

          <div style={{ marginTop: '2rem', minHeight: '60px', width: '100%', textAlign: 'center' }}>
            {!wheelSpun ? (
              <button className="btn-yes" onClick={handleSpin}>
                SPIN! 🎯
              </button>
            ) : (
              <div style={{ animation: 'fadeIn 1s ease', display: wheelResult ? 'block' : 'none' }}>
                <div style={{ background: '#fff', border: '2px dashed #db2777', padding: '15px', borderRadius: '12px', fontSize: '1.1rem', color: '#db2777', fontWeight: 'bold' }}>
                  {wheelResult}
                </div>
                <button 
                  onClick={() => setScene(8)}
      ...
      {/* SCENE 8: Animated Typewriter Letter */}
      {scene === 8 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>💌</div>
          <h2 style={{ fontSize: '1.5rem', color: '#db2777', marginBottom: '1.5rem', fontFamily: 'var(--font-handwriting)' }}>Dearest {recipient},</h2>
          ...
          <button 
            className="btn-yes pulse-element"
            onClick={() => setScene(9)}
      ...
      {/* SCENE 9: Virtual Gift Box & Confetti Finale */}
      {scene === 9 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <h2 style={{ fontSize: '2.5rem', color: '#db2777', fontFamily: 'var(--font-handwriting)', marginBottom: '2rem' }}>Surprise! 🎉</h2>
          ...
          <div 
            ...
            className={!boxOpened ? "pulse-element" : ""}
            style={{ fontSize: boxOpened ? '6rem' : '8rem', cursor: boxOpened ? 'default' : 'pointer', transition: 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)', transform: boxOpened ? 'scale(1.2)' : 'scale(1)' }}
          >
            {boxOpened ? '🥳' : '🎁'}
          </div>

          {!boxOpened ? (
            <p style={{ color: '#831843', fontSize: '1.2rem', marginTop: '2rem', fontWeight: 'bold' }}>Tap the gift to open!</p>
          ) : (
            <div style={{ animation: 'fadeIn 1s ease', marginTop: '2rem', textAlign: 'center' }}>
              <p style={{ fontSize: '1.4rem', color: '#db2777', fontWeight: 'bold', fontFamily: 'var(--font-handwriting)' }}>
                Have the most amazing year ahead! ❤️
              </p>
              <div style={{ marginTop: '1rem' }}>
                 <ReplayAndMarketingFooter onReplay={handleReplay} isPreview={isPreview} />
              </div>
            </div>
          )}
        </div>
