...
export default function EmotionalCreator({ templateId }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'notes', id), { creator_uid: deviceId, recipient_name: recipient.trim(), custom_message: message.trim(), image_urls: images, shagun_qr_url: null, custom_details: { ...details, relationship, vibe, experience_version: 1 }, is_paid: false, template: template.id, custom_slug: null, created_at: serverTimestamp(), expires_at: null });
  ...
  return <main className="shell emotional-creator-shell"><div className="emotional-creator-wrap">
    ...
    <section className="glass-card emotional-creator-card"><div className="emotional-stepper">{[1,2,3,4].map((number) => <span key={number} className={step >= number ? 'is-active' : ''}>{number}</span>)}</div>
        ...
        {step === 3 && <><div className="form-group"><label className="form-label">{labels[template.id]}</label><textarea className="form-input" rows="5" value={message} onChange={(e) => setMessage(e.target.value)} maxLength={1500} placeholder="Write from the heart…" /></div><div className="form-group"><label className="form-label">Choose your vibe</label><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>{vibes.map((item) => <button type="button" key={item} onClick={() => setVibe(item)} style={{ padding: '8px 12px', borderRadius: 99, border: '1px solid #f9a8d4', color: vibe === item ? '#fff' : '#9d174d', background: vibe === item ? '#be185d' : '#fff', cursor: 'pointer' }}>{item}</button>)}</div></div><div className="form-group"><label className="form-label">Photos (optional, up to {template.photoLimit})</label><CloudinaryUpload onUpload={addPhoto} />{images.length > 0 && <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>{images.map((url, index) => <button type="button" key={url} onClick={() => setImages(images.filter((_, i) => i !== index))} style={{ border: 0, background: 'none', cursor: 'pointer' }}><img src={url} alt="Selected memory" style={{ width: 64, height: 64, borderRadius: 10, objectFit: 'cover' }} /></button>)}</div>}</div></>}
        {error && <p style={{ color: '#be123c', fontWeight: 700, marginTop: 16 }}>{error}</p>}
        {step === 4 && <><h2>Ready for your private preview</h2><p className="text-muted">Your complete experience, payment summary, and voucher field are shown on the next page.</p></>}
